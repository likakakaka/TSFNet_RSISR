...
class Trainer():
    def __init__(self, args, loader, my_model, my_loss, ckp):
        ...
        if self.args.resume == 1:
            opti_load = torch.load(os.path.join(ckp.dir, 'optimizer.pt'))
            opti_load['param_groups'][0]['lr'] = args.lr
    ...
    def train(self):
        ...
        for batch, (lr, hr, file_names) in enumerate(self.loader_train):
            ...
            if self.args.model=='tsfnet':
                output1, output = self.model(lr)
                loss = self.loss(output, hr) + self.loss(output1, hr)
            else:
                sr = self.model(lr)
                    ...
                    if self.args.test_block:
                        ...
                        for iy in range(0, h, ip):
                            ...
                            for ix in range(0, w, ip):
                                ...
                                if self.args.model.lower() == 'tsfnet':
                                    sr_p, _ = self.model(lr_p)
                                else:
                    ...
                    if not no_eval:
                        ...
                        if self.args.test_y:
                            ...
                            for ii in range(sr.shape[0]):
                                sr[ii, :, :, 0] = utils.bgr2ycbcr(sr[ii, :, :, ::-1])
                                hr[ii, :, :, 0] = utils.bgr2ycbcr(hr[ii, :, :, ::-1])
                            sr = sr[:, :, :, 0]
                            hr = hr[:, :, :, 0]
                        ...
                        if crop_border == 0:
                            ...
                            if self.args.test_y:
                                cropped_hr = hr[:, crop_border:-crop_border, crop_border:-crop_border, np.newaxis]
                                cropped_sr = sr[:, crop_border:-crop_border, crop_border:-crop_border, np.newaxis]
                            else:
