...
path_original = 'G:\datasets\AID-dataset\val_small\';
dataset  = {'HR'};
ext = {'*.jpg', '*.png', '*.bmp', '*.tif'};
...
for idx_set = 1:length(dataset)
    ...
    for idx_im = 1:length(filepaths)
        ...
        for scale = scale_all
            ...
            folder_LR = fullfile(path_original,['LR_x', num2str(scale)]);
            if ~exist(folder_LR)
                mkdir(folder_LR)
            end
            % fn
            fn_LR = fullfile(path_original,['LR_x', num2str(scale)], [name_im]);
            imwrite(im_LR, fn_LR);
            folder_HR = fullfile(path_original,['HR_x', num2str(scale)]);
            if ~exist(folder_HR)
                mkdir(folder_HR)
            end
            fn_HR = fullfile(path_original,['HR_x', num2str(scale)], [name_im]);
            imwrite(im_HR, fn_HR);
        end
        fprintf('\n');
    end
